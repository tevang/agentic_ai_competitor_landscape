    ...
    def extract_candidates(
        ...
        if not docs:
            merged_from_cache = fuzzy_dedupe_candidates(
                self._filter_publisher_only(cached_priors + seed_candidates),
        ...
        merged_candidates = fuzzy_dedupe_candidates(
            self._filter_publisher_only(cached_priors + extracted_candidates + seed_candidates),
            threshold=self.config.dedupe.fuzzy_threshold,
        )
...
"""
    ...
    def _filter_publisher_only(self, candidates: list[Candidate]) -> list[Candidate]:
        """Remove candidates explicitly classified as article publishers rather than target vendors."""
